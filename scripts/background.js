...
async function isUrlBlocked(url) {
	try {
		...
		const { ['aive-blacklist-v1']: userBlk = [] } = await new Promise((resolve) => {
			try { chrome.storage.local.get(['aive-blacklist-v1'], (res) => resolve(res||{})); } catch { resolve({}); }
		});
		const builtin = ['yahoo.com'];
...
async function injectEnhancer(tab) {
	...
	try { await chrome.scripting.insertCSS({ target: { tabId: tab.id }, files: ["styles/aive/minimal.css"] }); } catch {}
	try { await chrome.scripting.executeScript({ target: { tabId: tab.id }, files: ["scripts/content-minimal.js"] }); } catch {}
	for (const fid of frameIds) {
		try { await chrome.scripting.insertCSS({ target: { tabId: tab.id, frameIds: [fid] }, files: ["styles/aive/minimal.css"] }); } catch {}
		try { await chrome.scripting.executeScript({ target: { tabId: tab.id, frameIds: [fid] }, files: ["scripts/content-minimal.js"] }); } catch {}
	}
...
chrome.commands.onCommand.addListener(async (command) => {
	...
	if (command === "inject_enhancer") {
		...
		try { await chrome.tabs.sendMessage(tab.id, { type: "TOGGLE_SLEEP" }); } catch {}
	}
});
